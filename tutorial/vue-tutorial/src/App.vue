...
let currentMessages = JSON.parse(JSON.stringify(i18n.global.getLocaleMessage(locale.value)));
...
function changeWord (key: string, word: string){
    const url = 'http://localhost:3000/Idioms/' + id; //change to + locale.value when api is ready

    currentMessages[key] =  word; 
    const newData =  {
      id: id,
      name: locale.value,
      vocabulary: currentMessages
    }
